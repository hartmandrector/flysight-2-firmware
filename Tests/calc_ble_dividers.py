...
HUM_ODR_MAP = {0: 0.0, 1: 1.0, 2: 7.0, 3: 12.0}
MAG_ODR_MAP = {0: 10.0, 1: 20.0, 2: 50.0, 3: 100.0}
ACCEL_ODR_MAP = {0: 0.0, 1: 12.0, 2: 26.0, 3: 52.0, 4: 104.0, 5: 208.0, 6: 416.0, 7: 833.0, 8: 1666.0, 9: 3333.0, 10: 6666.0, 11: 2.0}
GYRO_ODR_MAP = {0: 0.0, 1: 12.0, 2: 26.0, 3: 52.0, 4: 104.0, 5: 208.0, 6: 416.0, 7: 833.0, 8: 1666.0, 9: 3333.0, 10: 6666.0}

def odr_to_hz(odr_idx, sensor_type):
    """Convert ODR index to Hz for given sensor type"""
    if sensor_type == 'baro':
        return BARO_ODR_MAP.get(odr_idx, 0.0)
    elif sensor_type == 'hum':
        return HUM_ODR_MAP.get(odr_idx, 0.0)
    elif sensor_type == 'mag':
        return MAG_ODR_MAP.get(odr_idx, 0.0)
    elif sensor_type == 'accel':
        return ACCEL_ODR_MAP.get(odr_idx, 0.0)
    elif sensor_type == 'gyro':
        return GYRO_ODR_MAP.get(odr_idx, 0.0)
    return 0.0
...
def calculate_max_divider(hw_rate_hz):
    ...
    if hw_rate_hz <= 0:
        return 1
    return max(1, int(hw_rate_hz * 2))  # hw_rate / divider >= 0.5, so divider <= hw_rate * 2
...
def auto_calculate_dividers(config):
    ...
    gps_bandwidth = 0
    if config['enable_gnss'] and config['gnss_rate'] > 0:
        gps_rate_hz = 1000 // config['gnss_rate']
    ...
    sensors = [
        ('baro', config['enable_baro'], config['baro_odr'], config['ble_baro_divider'], BARO_PACKET_SIZE),
        ('hum', config['enable_hum'], config['hum_odr'], config['ble_hum_divider'], HUM_PACKET_SIZE),
        ('accel', config['enable_imu'], config['accel_odr'], config['ble_accel_divider'], ACCEL_PACKET_SIZE),
        ('gyro', config['enable_imu'], config['gyro_odr'], config['ble_gyro_divider'], GYRO_PACKET_SIZE),
        ('mag', config['enable_mag'], config['mag_odr'], config['ble_mag_divider'], MAG_PACKET_SIZE),
    ]
    
    for sensor_type, enabled, odr_idx, divider, packet_size in sensors:
        if not enabled or divider <= 0:
            continue
        if divider > 0:  # Manual divider
            hw_rate = odr_to_hz(odr_idx, sensor_type)
            ble_rate = int(hw_rate) // divider
    ...
    for sensor_type, enabled, odr_idx, divider, packet_size in sensors:
        if enabled and divider == 0:  # Auto mode
            hw_rate = odr_to_hz(odr_idx, sensor_type)
            ble_rate = int(hw_rate)  # divider=1
    ...
    if auto_bandwidth > remaining_budget and remaining_budget > 0:
        # Scaling required - calculate scale factor (match firmware rounding)
        scale_factor = float(auto_bandwidth) / float(remaining_budget)

        # Apply scaled dividers to all auto sensors
        for sensor_type, odr_idx, hw_rate, _ in auto_sensors:
            scaled_divider = int(scale_factor + 0.5)
            if scaled_divider < 1:
                scaled_divider = 1
        ...
        # Manual + GPS already consumed budget - set autos to minimum rate
        for sensor_type, _, hw_rate, _ in auto_sensors:
            min_rate_divider = calculate_max_divider(hw_rate)
            if sensor_type == 'baro':
                config['ble_baro_divider'] = min_rate_divider
            elif sensor_type == 'hum':
                config['ble_hum_divider'] = min_rate_divider
            elif sensor_type == 'accel':
                config['ble_accel_divider'] = min_rate_divider
            elif sensor_type == 'gyro':
                config['ble_gyro_divider'] = min_rate_divider
            elif sensor_type == 'mag':
                config['ble_mag_divider'] = min_rate_divider
    else:
...
def calculate_bandwidth(config):
    ...
    gps_bw = 0
    if config['enable_gnss'] and config['gnss_rate'] > 0:
        gps_rate = 1000 // config['gnss_rate']
    ...
    baro_rate = (int(odr_to_hz(config['baro_odr'], 'baro')) // baro_div) if config['enable_baro'] else 0
    baro_bw = baro_rate * BARO_PACKET_SIZE
    
    hum_rate = (int(odr_to_hz(config['hum_odr'], 'hum')) // hum_div) if config['enable_hum'] else 0
    hum_bw = hum_rate * HUM_PACKET_SIZE
    
    accel_rate = (int(odr_to_hz(config['accel_odr'], 'accel')) // accel_div) if config['enable_imu'] else 0
    accel_bw = accel_rate * ACCEL_PACKET_SIZE
    
    gyro_rate = (int(odr_to_hz(config['gyro_odr'], 'gyro')) // gyro_div) if config['enable_imu'] else 0
    gyro_bw = gyro_rate * GYRO_PACKET_SIZE
    
    mag_rate = (int(odr_to_hz(config['mag_odr'], 'mag')) // mag_div) if config['enable_mag'] else 0
...
def print_config(config, title):
    ...
    if config['enable_gnss'] and config['gnss_rate'] > 0:
        ...
        print("GPS Rate:      DISABLED")
    
    print("\nSensor ODRs:")
    print(f"  Baro:        {odr_to_hz(config['baro_odr'], 'baro'):.1f} Hz (ODR={config['baro_odr']}) {'EN' if config['enable_baro'] else 'DIS'}")
    print(f"  Humidity:    {odr_to_hz(config['hum_odr'], 'hum'):.1f} Hz (ODR={config['hum_odr']}) {'EN' if config['enable_hum'] else 'DIS'}")
    print(f"  Accel:       {odr_to_hz(config['accel_odr'], 'accel'):.1f} Hz (ODR={config['accel_odr']}) {'EN' if config['enable_imu'] else 'DIS'}")
    print(f"  Gyro:        {odr_to_hz(config['gyro_odr'], 'gyro'):.1f} Hz (ODR={config['gyro_odr']}) {'EN' if config['enable_imu'] else 'DIS'}")
    print(f"  Mag:         {odr_to_hz(config['mag_odr'], 'mag'):.1f} Hz (ODR={config['mag_odr']}) {'EN' if config['enable_mag'] else 'DIS'}")
...
Examples:
  ...
  %(prog)s --gps-ms 40 --accel-odr 6 --gyro-odr 6
    ...
    parser.add_argument('--baro-odr', type=int, default=2, 
                        help='Baro ODR: 0=off, 1=1Hz, 2=10Hz, 3=25Hz, 4=50Hz, 5=75Hz, 6=100Hz, 7=200Hz (default: 2)')
    parser.add_argument('--hum-odr', type=int, default=1, 
                        help='Hum ODR: 0=off, 1=1Hz, 2=7Hz, 3=12Hz (default: 1)')
    parser.add_argument('--accel-odr', type=int, default=1, 
                        help='Accel ODR: 0=off, 1=12Hz, 2=26Hz, 3=52Hz, 4=104Hz, 5=208Hz, 6=416Hz, 7=833Hz, 8=1666Hz, 9=3333Hz, 10=6666Hz, 11=2Hz (default: 1)')
    parser.add_argument('--gyro-odr', type=int, default=1, 
                        help='Gyro ODR: 0=off, 1=12Hz, 2=26Hz, 3=52Hz, 4=104Hz, 5=208Hz, 6=416Hz, 7=833Hz, 8=1666Hz, 9=3333Hz, 10=6666Hz (default: 1)')
    parser.add_argument('--mag-odr', type=int, default=0, 
                        help='Mag ODR: 0=10Hz, 1=20Hz, 2=50Hz, 3=100Hz (default: 0)')
    parser.add_argument('--disable-gnss', action='store_true', help='Disable GNSS contribution')
    parser.add_argument('--disable-baro', action='store_true', help='Disable barometer contribution')
    parser.add_argument('--disable-hum', action='store_true', help='Disable humidity contribution')
    parser.add_argument('--disable-imu', action='store_true', help='Disable accel/gyro contribution')
    parser.add_argument('--disable-mag', action='store_true', help='Disable magnetometer contribution')
    ...
    config = {
        ...
        'enable_gnss': not args.disable_gnss,
        'enable_baro': not args.disable_baro,
        'enable_hum': not args.disable_hum,
        'enable_imu': not args.disable_imu,
        'enable_mag': not args.disable_mag,
